...
begin
  ...
  begin
    ...
    repeat
      Inc(i);
    until Form_Main.SG_FixList.Cells[0,i]='';
    Form_Main.SG_FixList.Cells[0,i] := IntToStr(Round(X*lMag));
    Form_Main.SG_FixList.Cells[1,i] := IntToStr(Round(Y*lMag));
    Form_Main.SG_FixList.Cells[2,i] := '0';
  end;
  ...
  begin
    ...
    TI := SPoint[Y*lMag,X*lMag].BaseP ;
    CI := StrToInt(Form_Main.Edit_PV.Text);
    NumP:=1;
    SelPData[NumP].X := X*lMag;
    SelPData[NumP].Y := Y*lMag;
    SelData[Y*lMag,X*lMag] := true;
    ...
    repeat
      ...
      begin
        lX := SelPData[k].X;
        lY := SelPData[k].Y;
        if (lX>0) and (lY>0) and (lY<PH-1) and (lX<PW-1) then
        for j:=lY-1 to lY+1 do
          for i:=lX-1 to lX+1 do
            if (j-ly)*(i-lx)=0 then
            if (SPoint[j,i].BaseP = TI) and not(SelData[j,i]) then
             begin
               Inc(NumP);
               SelPData[NumP].X := i;
               SelPData[NumP].Y := j;
    ...
    for k:=1 to NumP do
      SPoint[SelPData[k].Y,SelPData[k].X].BaseP := CI;
    Form_Main.Draw_Unwrapping(Sender);
  end;
